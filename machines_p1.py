...
class P1():
                ...
                if count >= 3:
                    for i in range(4):
                        if subgrid[i] == 1:
                            ...
                            for j in range(4):
                                if len(set(char[j] for char in characteristics)) == 1:
                                    return True
        return False
    ...
    def check_three(self):
        ...
        if self.check_line_three([self.board[i][i] for i in range(BOARD_ROWS)]) or self.check_line([self.board[i][BOARD_ROWS - i - 1] for i in range(BOARD_ROWS)]):
            return True
